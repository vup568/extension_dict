...
| **Knowledge Operations** (6) | `source_manifests`, `source_records`, `editorial_mappings`, `knowledge_releases`, `knowledge_release_manifests`, `resource_revisions` | Provenance, versioning, import pipeline |
...
- **Immutable Knowledge Release:** `knowledge_releases` có `published_at`, `is_current` với partial unique index (DATA-005).
...
| **Domain Entities** | [`src/domain/Entities/`](src/domain/Entities/) | 17 C# POCO classes — thuần C# 14, không phụ thuộc EF Core |
| **EF Core Configurations** | [`src/infra/Persistence/Configurations/`](src/infra/Persistence/Configurations/) | 17 Fluent API configurations ánh xạ Entity → PostgreSQL |
| **DbContext** | [`src/infra/Persistence/AppDbContext.cs`](src/infra/Persistence/AppDbContext.cs) | EF Core DbContext chính với 17 DbSet |
